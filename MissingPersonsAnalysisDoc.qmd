...
This visualization compares missing-person counts across age groups for Pennsylvania and Ohio. In both states, the highest number of reported cases falls within the 20–40 age group, followed by individuals aged 40–60. Fewer cases are observed among individuals below 20 and above 60. While Pennsylvania generally reports slightly higher counts than Ohio across all age groups, the overall age distribution pattern is highly consistent between the two states, suggesting similar demographic trends rather than state-specific differences.
...
This visualization shows missing-person counts by biological sex for Pennsylvania and Ohio. Both states exhibit a relatively balanced distribution between males and females, with only modest differences in total counts. Pennsylvania reports slightly higher numbers overall, but the proportional relationship between biological sex categories remains similar across states. This indicates that missing-person reports are not strongly skewed toward a single biological sex in either Pennsylvania or Ohio.
...
This lollipop plot displays missing-person counts across race and ethnicity categories for Pennsylvania and Ohio. In both states, White / Caucasian individuals represent the largest share of reported missing-person cases, followed by Black / African American individuals. All other racial and ethnic groups appear at substantially lower counts. The similarity in distributions between the two states suggests that these patterns may reflect broader population composition and reporting practices rather than unique state-level effects.

```{r}
### race distribution by state - lollipop plot
library(ggplot2)
...
Across all combinations of race/ethnicity, biological sex, and age group, the demographic group with the **highest number of reported missing-person cases** is **White / Caucasian females between the ages of 20 and 40**, with a total of **160 cases**. This result aligns with the broader age and race patterns observed in the demographic visualizations and highlights young adult White/Caucasian females as the single most frequently reported demographic group in the dataset.
...
1.  Lollipop chart in ggplot2. (2021, February 23). R CHARTS \| a Collection of Charts and Graphs Made with the R Programming Language. https://r-charts.com/ranking/lollipop-chart-ggplot2/
2.  Making Maps with R. (n.d.). *Eriq Ande*. Retrieved December 15, 2025, from https://eriqande.github.io/rep-res-web/lectures/making-maps-with-R.html